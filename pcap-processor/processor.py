...
for message in job_consumer:
    ...
    if conn_log_path.exists():
        print("Found conn.log. Publishing results to 'voip_logs' topic...", flush=True)
        with open(conn_log_path) as f:
            for line in f:
                ...
                try:
                    log_data = json.loads(line)
                    log_data['fields'] = {'log_type': 'conn'}
                    results_producer.send('voip_logs', log_data)
                except json.JSONDecodeError:
                    print(f"Skipping malformed line in conn.log: {line.strip()}", flush=True)
